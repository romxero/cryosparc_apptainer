...
# envs
THIS_USER=$(whoami)
THIS_USER_SUFFIX=${USER_SUFFIX:-'czbiohub.org'}
...
create_account ${ACCOUNT} "${CRYOSPARC_PASSWORD}" "${THIS_USER}"
# add additional
...
if [ "${CRYOSPARC_LOCAL_WORKER}" == "1" ]; then
  ...
  export CRYOSPARC_CACHE_DIR=${CRYOSPARC_CACHE_DIR:-"/tmp/cryosparc"}
  mkdir -p ${CRYOSPARC_CACHE_DIR}
  NOGPU=""
  if [ ! -z $CRYOSPARC_WORKER_NOGPU ]; then
    NOGPU="--nogpu"
  fi
  SSD_OPTS="--ssdpath ${CRYOSPARC_CACHE_DIR}"
  if [ ! -z $CRYOSPARC_WORKER_NOSSD ]; then
    SSD_OPTS="--nossd"
  fi
  #  if slurm cpu var set, set cpus, otherwise yolo
  if [ -z ${SLURM_NPROCS+x} ]; then
    ...
    ${CRYOSPARC_WORKER_DIR}/bin/cryosparcw connect --cpus $SLURM_NPROCS --worker ${CRYOSPARC_MASTER_HOSTNAME} --master ${CRYOSPARC_MASTER_HOSTNAME} --port ${CRYOSPARC_BASE_PORT} ${SSD_OPTS} ${NOGPU}
  fi
fi
